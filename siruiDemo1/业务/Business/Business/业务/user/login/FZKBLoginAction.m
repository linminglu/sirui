...
@implementation FZKBLoginAction
...
- (void)progress:(FZKActionResult *)result
{
    ...
    FZKBLoginModel *model = [FZKBLoginModel mj_objectWithKeyValues:result.paramters[@"option"]];
//    FZKBCustomerModel *customer = model.customer;
    [model archive];
...
//    if (model.customer.cars.count) {
//        id car = model.customer.cars[0];
//   
//    }
    
// [[NSUserDefaults standardUserDefaults] setObject:@0 forKey:@"kSelectCarId"];
    kSaveCurrentCarModleId(0);

    
    //    NSLog(@"login parameters %@",self.parameters);
    
    //登录成功用户名 密码保存至钥匙串
    [FZKBKeychain updateUserName:[self.parameters objectForKey:@"input1"]];
    [FZKBKeychain updatePassword:[self.parameters objectForKey:@"input2"]];
 ...
 */
+ (void)loginActionWithInput1:(NSString *)input1 input2:(NSString *)input2 success:(Action1)success fail:(Action1)fail
{
    FZKBLoginAction *work =[[FZKBLoginAction alloc] init];
    

    [work loginActionWithInput1:input1 input2:input2];
    ...
    [work onSuncc:^(FZKActionResult *result) {
        ...
        success(result.paramters[@"option"]);
