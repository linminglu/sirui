...
#import "FZKLocationManager.h"
#define PI 3.141592653
@interface FZKLocationManager()
@property (nonatomic,strong) CLGeocoder *geocoder;

@end

@implementation FZKLocationManager
...
- (void)reverseGeocodeWithlatitude:(CLLocationDegrees )latitude longitude:(CLLocationDegrees)longitude success:(void(^)(NSString *address))success failure:(void(^)())failure
{
    ...
    [self.geocoder reverseGeocodeLocation:loc completionHandler:^(NSArray *placemarks, NSError *error) {
        if (error) {
            ...
            if (success) {
//                success(pm.name);
                success([NSString stringWithFormat:@"%@%@%@",pm.administrativeArea,pm.subLocality, pm.name]);
            }
        }
    }];
}
